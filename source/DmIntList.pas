...
type
  VarArrayOfInt = array of Integer;


type
  TDmIntegerList = class
  private
    FList: VarArrayOfInt;
    FSorted: Boolean;
    FCapacity: Integer;
    ...
    function GetLast(var idx: Integer): Integer;
    function GetFirst(var idx: Integer): Integer;
    ...
    function _GetItemsInternal(idx: integer): Integer;
    function _XValue(avalue: Integer): Integer;
    function _YValue(avalue: Integer): Integer;
{$IFDEF WITHTEST}
    procedure IntCheck;
{$ENDIF}
    procedure _Insert(pos: Integer; avalue: Integer);
    function GetCapacity: integer;
    procedure _append(avalue: Integer);
  public
    property Capacity: integer read GetCapacity;
    procedure Add(avalue: Integer); overload;
    procedure Add(list: TDmIntegerList); overload;
    procedure Remove(avalue: Integer);
    property Duplicates: TDuplicates read GetDuplicates write SetDuplicates;
    property Items[idx: integer]: Integer read GetItems; default;
    ...
    function CopyArray: VarArrayOfInt;
...
function TDmIntegerList._NextSpace(idx: Integer): Integer;
begin
  if idx >= 0 then
    result := idx
  else
...
function TDmIntegerList._XValue(avalue: Integer): Integer;
begin
...
function TDmIntegerList._YValue(avalue: Integer): Integer;
begin
...
procedure TDmIntegerList._Insert(pos: Integer; avalue: Integer);
var
  k: Integer;
begin
...
procedure TDmIntegerList._append(avalue: Integer);
begin
  if FSorted then
...
procedure TDmIntegerList.Add(avalue: Integer);
var
  i: Integer;
begin
...
procedure TDmIntegerList.Delete(idx: integer);
var
  j, i: Integer;
begin
  if FCount = 0 then
...
function TDmIntegerList.Find(avalue: Integer; var idx: integer): boolean;
var
...
begin
  ...
  if IntegerCompare(avalue, GetLast(last)) >= 0 then
  begin
    idx := last;
  ...
  if IntegerCompare(avalue, GetFirst(first)) <= 0 then
  begin
    idx := first;
    result := avalue = FList[idx];
    if not Result then
  ...
  begin
    ...
    begin
      ...
      c := IntegerCompare(avalue, _GetItemsInternal(mid));
      if c <= 0 then
      begin
        if c = 0 then
        begin
...
function TDmIntegerList.GetCapacity: integer;
begin
  result := FCapacity;
end;

function TDmIntegerList.GetCount: integer;
begin
  result := fcount;
end;

function TDmIntegerList.GetDuplicates: TDuplicates;
begin
  result := FDuplicates;
end;

function TDmIntegerList.GetItems(idx: integer): Integer;
var
  i: Integer;
begin
  i := NearesteIdx(idx);
...
function TDmIntegerList.CopyArray: VarArrayOfInt;
var
  j, i: Integer;
begin
...
procedure TDmIntegerList.Remove(avalue: Integer);
var
  i: Integer;
begin
...
procedure TDmIntegerList.SetDuplicates(const Value: TDuplicates);
begin
...
procedure TDmIntegerList.SetSorted(const Value: boolean);
begin
...
procedure TDmIntegerList.Sort;
  ...
  procedure Partition(var A: VarArrayOfInt; First, Last: integer);
  var
    Right, Left: integer;
    V: integer;
  begin
  ...
  procedure QuickSort(var List: VarArrayOfInt);
  var
    First, Last: integer;
  begin
    First := 0;
...
constructor TDmIntegerList.Create;
begin
  Clear;
  FDuplicates := dupAccept;
end;

destructor TDmIntegerList.Destroy;
begin
  Clear;
  inherited;
end;

procedure TDmIntegerList.Clear;
begin
  FCapacity := 0;
  FCount := 0;
  FLastActualIdx := -1;
...
function TDmIntegerList.NearesteIdx(idx: Integer): Integer;
var
  i: Integer;
begin
...
function TDmIntegerList.GetFirst(var idx: Integer): Integer;
begin
  if FCount > 0 then
  begin
    idx := NearesteIdx(0);
...
function TDmIntegerList.GetLast(var idx: Integer): Integer;
begin
  if FCount > 0 then
  begin
...
function TDmIntegerList._GetItemsInternal(idx: integer): Integer;
begin
  if FCount = 0 then
    result := NULL_VALUE
  else
...
{$IFDEF WITHTEST}
procedure TDmIntegerList.IntCheck;
var
...
function TDmIntegerList.Exists(avalue: Integer): Boolean;
var
  i: Integer;
begin
...
procedure TDmIntegerList.Add(list: TDmIntegerList);
var
  i: Integer;
  a: VarArrayOfInt;
begin
...
function TDmIntegerList.Text: string;
var
  j, i: Integer;
{$IFDEF WITHTEST}
  k: Integer;
